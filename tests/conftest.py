...
def instruction_pool(real_report):
    from memory_cfg.synth import harvest_instruction_pool, _FALLBACK_POOL
    return harvest_instruction_pool(real_report) + list(_FALLBACK_POOL)


@pytest.fixture(scope="session")
def synthetic_report(instruction_pool):
    from memory_cfg.synth import generate_report, PROFILES_BY_NAME
...
def tiny_dataset(tmp_path_factory, real_report):
    ...
    from memory_cfg.data_tokenizer import write_corpus_from_dir
    from memory_cfg.tokenizer_train import train_tokenizer
